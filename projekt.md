...
23. `uninstall_app`: Entfernt ein Programm über den *eigenen* Deinstaller des Herstellers (Windows-Uninstall-Registry, `.app`-Bundle, `send2trash` auf macOS) — nach CONFIRM auf dem HUD, mit Anzeige des exakten Kommandos. Systembestandteile (Windows, Treiber, Runtimes) werden vor dem Banner verweigert; gibt es keinen verlässlichen Deinstaller, sagt es das, statt Ordner zu löschen.
24. `restart_app`: Schließt und startet eine App über denselben Controller (`close` → `open`), beide Hälften verifiziert. War die App nicht offen, wird sie gestartet und die Antwort sagt genau das; schließt sie sich nicht, startet niemals eine zweite Kopie.
25. `window_control`: Listet/fokussiert/minimiert/maximiert/stellt Fenster anhand von App-Name oder Fenstertitel wieder her — dieselbe Implementierung wie `computer_control`. Sendet niemals Tastenanschläge ans gerade fokussierte Fenster und schließt nie etwas (`close_app` ist dafür zuständig).
26. `system_status`: fasst CPU, RAM, Datenträger, Netzwerk, Akku, Laufzeit und die laufenden Aufgaben in einer Zeile zusammen.
27. `agent_task`: Multi-Step-Agent — führt ein Ziel („öffne Spotify und suche Jazz“) als Schrittfolge über den zentralen Tool-Dispatcher aus (Plan → Execute → Verify); stoppt ehrlich beim ersten unverifizierten Schritt.
...
- **Installations-Gate für alles, was installiert**: `game_updater` (Installieren/Aktualisieren) und `uninstall_app` nutzen dieselbe Schranke (`core/install_safety.guard`) wie Herunterfahren/Neustart — Banner mit CONFIRM/CANCEL, fail-closed ohne gebundenes Interface, systemeigene Ziele (Windows, Treiber, Runtimes) werden vorher verweigert. Deinstallationen laufen ausschließlich über den registrierten Hersteller-Deinstaller, nie über ein selbstgebautes `rm`/`del`.
- **Zentrale Aufgaben-Anzeige (`core/tasks.py`)**: Downloads, Installationen, Timer und Agent-Läufe melden sich bei einem Register, das das HUD als **TASKS**-Panel anzeigt (Bytes, Prozent, Geschwindigkeit, ETA, Zustand, Abbrechen). Ein Download ohne Fortschrittsmeldung zeigt einen unbestimmten Balken statt erfundener Zahlen; der Playwright-Browser-Download wird währenddessen aus dem Cache auf der Platte gemessen.
- **Plugins bremsen den Start nicht**: Der Loader liest einen Plugin-Body, statt ihn auszuführen, wo das gefahrlos möglich ist (Importe, Definitionen, Zuweisungen) und importiert ihn beim ersten Aufruf. Ein geparktes Plugin (`PARKED_PLUGINS`) wird erkannt und im Plugin-Manager gelistet, aber weder als Tool angeboten noch gestartet — `telegram_remote` ist derzeit geparkt, nicht gelöscht.
