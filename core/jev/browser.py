"""
Observed actions through Chrome DevTools Protocol (CDP).
Uses browser-harness to connect to Chrome with zero per-step process overhead.
"""
...
_SNAPSHOT_PATH = Path(__file__).with_name("snapshot.js")
READ_STATE = _SNAPSHOT_PATH.read_text(encoding="utf-8") if _SNAPSHOT_PATH.exists() else ""
...
def fingerprint(state: dict[str, Any]) -> str:
    content = {k: state[k] for k in ("url", "text", "actions", "scroll") if k in state}
    return hashlib.sha256(json.dumps(content, sort_keys=True).encode()).hexdigest()


def browser_operation(request: dict[str, Any]) -> dict[str, Any]:
...
class Browser:
    def __init__(self, url: str):
        if not _HARNESS_AVAILABLE:
            raise RuntimeError("browser-harness is required for Jev browser control.")
        ensure_daemon()
    ...
    def fresh(self, page: dict[str, Any], action: Optional[dict[str, Any]] = None) -> bool:
        if action is not None and action.get("kind") in {"click", "select"}:
            ...
            return current == [page.get("page_key"), page.get("guards", {}).get(str(node))]
