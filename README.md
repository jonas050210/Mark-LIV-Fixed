...
On Windows, MARK LIV subscribes to the OS's own window events (WinEventHook): a window appearing, being re-titled, or moving wakes the waiting command instantly instead of on the next poll tick, and the window list is cached between desktop events. Where the hook cannot be installed, the polling behaviour from before continues unchanged.

Closing the assistant (`shutdown_jarvis`, the tray menu, or voice) is immediate and never asks for confirmation — it saves the session, says goodbye, and exits; simply start the app again to bring it back. Powering off or restarting the **computer** is a different action and keeps its on-screen confirmation.
...
Per-application volume works through the Windows mixer: say `list app volumes`, then `set Spotify to 50`, `make Discord quieter`, or `mute Chrome` — one application's volume changes without touching the system volume or any other app, and `undo` puts it back. Matching is by application process name, so browser tabs share their browser's sessions (say `Chrome`, not a page name).
...
All discovered actions now pass through one registry contract. Results have explicit `succeeded`, `failed`, `busy`, `cancelled`, `forbidden`, `confirmation_pending`, `unavailable`, and `timed_out` states; handlers keep their old string API only at the Gemini boundary. The registry also owns per-action deadlines, bounded legacy-worker capacity, trusted/admin checks, confirmation metadata, and the dashboard capability manifest. Packaged action source is size-, ownership-, permission-, link-, and descriptor-checked before execution. The settings drawer has a search field that filters its controls, and the confirmation banner can be answered with the **Y**/**N** keys as well as its buttons.
