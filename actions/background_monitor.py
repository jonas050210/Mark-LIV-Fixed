...
def _save(monitors: dict) -> None:
    # Through the public API: atomic tmp+rename write with a backup, under
    # the store lock — instead of reaching into the manager's privates and
    # writing the file directly. Trim only drops {"value": ...} entries, so
    # the monitors dict passes through untouched.
