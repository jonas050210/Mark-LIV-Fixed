"""Appearance panel: accent colour wheel, avatar and identity settings.
...
class HueWheel(QWidget):
    ...
    def __init__(self, initial_hex: str = _default_accent(), parent=None):
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="JARVIS", user_name="",
        ...
        """)
        df_btn.clicked.connect(lambda: self._set_color(_default_accent()))
        clr_hdr.addWidget(df_btn)
        lay.addLayout(clr_hdr)

        self._initial_color = (ui_color or _default_accent()).strip().lower()
    ...
    def _save(self):
        ...
        self.saved.emit(name, user, self._sel_color or _default_accent(), self._sel_voice)
        self.hide()
